...
export function VolumeControl({ volume, isMuted, onVolumeChange, onToggleMute }: VolumeControlProps) {
  return (
    <div className="flex items-center gap-2 sm:gap-3 w-full group">
      ...
      {/* Volume % */}
      <span className="text-[10px] sm:text-xs text-[var(--text-secondary)] w-6 sm:w-8 text-right font-mono">{volume}</span>
    </div>
  );
}